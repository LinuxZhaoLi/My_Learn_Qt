...
void Dialog::rotateWidgets()
{
    ...
    foreach (QWidget *widget, rotableWidgets)  // 遍历队列
...
{
    rotableGroupBox = new QGroupBox(tr("Rotable Widgets"));// 创建一个分组
    ...
    rotableGroupBox->setLayout(rotableLayout); // 布局填充组件
...
void Dialog::createOptionsGroupBox()
{
    ...
    optionsLayout->setColumnStretch(2, 1); // 设置弹簧
    optionsGroupBox->setLayout(optionsLayout);
}

void Dialog::createButtonBox()
{
    buttonBox = new QDialogButtonBox;  //按钮盒子
